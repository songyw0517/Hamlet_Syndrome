...
public class FifthLayout_USA extends JPanel {
	//instance data
		...
		public FifthLayout_USA(JFrame thirdLayout, JPanel prePanel, JPanel firstPanel) {//생성자
	...
	private class ButtonListener implements ActionListener{//버튼 리스너 클레스
		public void actionPerformed(ActionEvent e) {
			...
			if(B==Rice) {//밥이 눌렸을 경우
				...
				new SixthLayout_USA(fifthLayout, primary, 1, firstPanel);
			}else if(B==Noodle) {//면이 눌렸을 경우
				setVisible(false);
				primary.setVisible(false);
				new SixthLayout_USA(fifthLayout, primary,2, firstPanel);
			}else if(B==Etc) {//기타가 눌렸을 경우
				primary.setVisible(false);
				new SixthLayout_USA(fifthLayout, primary, 3, firstPanel);
